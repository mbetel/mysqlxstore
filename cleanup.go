...
func (m *MySQLXStore) cleanup(interval time.Duration, quit <-chan struct{}, done chan<- struct{}) {
	...
	for {
		...
		case <-ticker.C:
			...
			err := m.deleteExpired()
			if err != nil {
				log.Printf("mysqlxstore: unable to delete expired sessions: %v", err)
			}
		}
	}
}
...
func (m *MySQLXStore) deleteExpired() error {
