/*
Gorilla Sessions backend for Sqlx
Based on https://github.com/srinathgs/mysqlstore
...
func NewMySQLXStore(endpoint string, tableName string, path string, maxAge int, keyPairs ...[]byte) (*MySQLXStore, error) {
	db, err := sqlx.Open("mysql", endpoint)
	if err != nil {
		return nil, err
	}

	return NewMySQLXStoreFromConnection(db, tableName, path, maxAge, keyPairs...)
}

func NewMySQLXStoreFromConnection(db *sqlx.DB, tableName string, path string, maxAge int, keyPairs ...[]byte) (*MySQLXStore, error) {
	...
	stmtInsert, stmtErr := db.Preparex(insQ)
	if stmtErr != nil {
		return nil, stmtErr
	}

	delQ := "DELETE FROM " + tableName + " WHERE id = ?"
	stmtDelete, stmtErr := db.Preparex(delQ)
	if stmtErr != nil {
		return nil, stmtErr
	}
	...
	updQ := "UPDATE " + tableName + " SET session_data = ?, created_on = ?, expires_on = ? " +
		"WHERE id = ?"
	stmtUpdate, stmtErr := db.Preparex(updQ)
	if stmtErr != nil {
		return nil, stmtErr
	}
	...
	selQ := "SELECT id, session_data, created_on, modified_on, expires_on from " +
		tableName + " WHERE id = ?"
	stmtSelect, stmtErr := db.Preparex(selQ)
	if stmtErr != nil {
		return nil, stmtErr
	}
...
func (m *MySQLXStore) insert(session *sessions.Session) error {
...
func (m *MySQLXStore) save(session *sessions.Session) error {
...
func (m *MySQLXStore) load(session *sessions.Session) error {
